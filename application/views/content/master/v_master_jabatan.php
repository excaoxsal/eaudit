...
<script type="text/javascript">
      ...
      submitHandler: function () {
        ...
        Toast.fire({
          ...
          if (result.isConfirmed) {
            prosesDataMaster("<?= base_url('master/jabatan/post') ?>").then( function(data){
  ...
  function hapus(id) {
    Toast.fire({
        ...
        if (result.isConfirmed) {
            prosesDataMaster(`<?= base_url() ?>master/jabatan/hapus?id=${id}`).then( function(data){
              var obj = JSON.parse(data); 
              if(obj.status=='OK'){
                ToastTopRightTimer.fire({ icon: 'success', title: 'Proses Berhasil! '+obj.msg })
                _reloadData();
  ...
  const update = (id) => {
    fetch(`<?= base_url() ?>master/jabatan/jabatan_json?id=${id}`, {method:'POST'})
...
var KTDatatableJsonRemoteDemo = {
  init: function() {
    ...
    t = $("#datatable").KTDatatable({
      data: {
        type: "remote",
        source: '<?= base_url() ?>master/jabatan/jabatan_json',
        pageSize: 10
      },
      layout: {
        scroll: !1,
