...
<script type="text/javascript">
      ...
      submitHandler: function () {
        ...
        Toast.fire({
          ...
          if (result.isConfirmed) {
            prosesDataMaster("<?= base_url('master/user/post') ?>").then( function(data){
  ...
  function action(act, id_user) 
  {
    if(act=='reset_password') {
      url       = "<?= base_url() ?>master/user/action/reset_password?id="+id_user;
      msg_conf  = 'Reset password user ini?';
    }
    else if(act=='aktif') {
      url ="<?= base_url() ?>master/user/action/aktif?id="+id_user;
      msg_conf  = 'Aktifkan user ini?';
    }
    else if(act=='nonaktif') {
      url ="<?= base_url() ?>master/user/action/nonaktif?id="+id_user;
  ...
  const update = (id) => {
    fetch(`<?= base_url() ?>master/user/user_json?id=${id}`, {method:'POST'})
...
var KTDatatableJsonRemoteDemo = {
  init: function() {
    ...
    t = $("#datatable").KTDatatable({
      data: {
        type: "remote",
        source: '<?= base_url('master/user/user_json') ?>',
        pageSize: 10
      },
      layout: {
        scroll: !1,
