...
class User extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $data['content']        = 'content/master/v_master_user';
        $this->show($data);
    }
    ...
    function user_json()
    {
        ...
        if($this->is_auditor() && $this->input->server('REQUEST_METHOD') === 'POST') echo json_encode($this->master_act->user($where));
    }
    ...
    public function action($act)
    {
        ...
        if ($id) {
            ...
            elseif($act=='reset_password') {
                $random_int         = random_int(100000, 999999);
                $data['RANDOM_INT'] = $random_int; 
                // $data['PASSWORD']   = password_hash($random_int, PASSWORD_DEFAULT); 
